...
def save_contacts(project_id: str, links: dict, coingecko_url: str):
    rows = []
    if coingecko_url:
        rows.append({"project_id": project_id, "platform": "CoinGecko",   "value": coingecko_url, "role": "Listing"})
    if links.get("website"):
        rows.append({"project_id": project_id, "platform": "Website",     "value": links["website"],  "role": "Official"})
    if links.get("twitter"):
        rows.append({"project_id": project_id, "platform": "X / Twitter", "value": links["twitter"],  "role": "Social"})
    if links.get("telegram"):
        rows.append({"project_id": project_id, "platform": "Telegram",    "value": links["telegram"], "role": "Community"})
    ...
    for row in rows:
        try:
            ...
            if "duplicate" not in str(e).lower() and "unique" not in str(e).lower():
                print(f"    [WARN contacts] {row['platform']}: {e}")
